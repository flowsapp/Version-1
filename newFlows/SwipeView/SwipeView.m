...
//- (CustomSwipeview *)itemViewAtIndex:(NSInteger)index
{
    return _itemViews[@(index)];
}

- (CustomSwipeview*)itemCustomViewAtIndex:(NSInteger)index{
    return _itemViews[@(index)];
}
...
- (void)setFrameForView:(CustomSwipeview *)view atIndex:(NSInteger)index
{
    if (self.window)
    {
...
{
    for (CustomSwipeview *view in self.visibleItemViews)
    {
...
- (CustomSwipeview *)dequeueItemView
{
    //UIView *view = [_itemViewPool anyObject];
    CustomSwipeview *view = [_itemViewPool anyObject];
    if (view)
    {
...
- (CustomSwipeview *)loadViewAtIndex:(NSInteger)index
{
    //UIView *view = [_dataSource swipeView:self viewForItemAtIndex:index reusingView:[self dequeueItemView]];
    CustomSwipeview *view = [_dataSource swipeView:self viewForItemAtIndex:index reusingView:[self dequeueItemView]];
    if (view == nil)
    {
        view = [[CustomSwipeview alloc] init];
    }
...
- (void)updateItemSizeAndCount
{
    ...
    else if (_numberOfItems > 0)
    {
        ...
        CustomSwipeview *view = [[self visibleItemViews] lastObject] ?: [_dataSource swipeView:self viewForItemAtIndex:0 reusingView:[self dequeueItemView]];
